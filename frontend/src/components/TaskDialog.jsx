  ...
  const TaskDialog = ({ open, onClose, onAdd, onEdit, users, task }) => {
    ...
    useEffect(() => {
      if (task) {
        setTaskData({
          ...
          assignee: users.find(user => user.id === task.assignee.id) || users[0],
        });
      } else {
    ...
    const getTypeIcon = (type) => {
      switch (type) {
        ...
        case 'story': return <AutoStoriesIcon color="primary" />;
      ...
      >
        <DialogTitle sx={{ 
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            {task ? getTypeIcon(task.type) : <AssignmentIcon />}
                    ...
                    >
                      {[1, 2, 3, 5, 8, 13].map(point => (
                        <MenuItem key={point} value={point}>
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                            <SpeedIcon />
                            <Typography>{point} {point === 1 ? 'point' : 'points'}</Typography>
                          </Box>
                        </MenuItem>
                      ))}
                    </Select>
